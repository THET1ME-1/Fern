...
**Готовые наборы: 500 слов, шесть языков.** Английский лежит в
`assets/seed/en.json` (он же набор по умолчанию), остальные — в
`assets/starter/<code>.json`: es, de, fr, it, ru. Раскладка у всех одна:
«Первые слова» 180, «Глаголы» 140, «Еда и напитки» 100, «Одежда» 80. Карточка
несёт перевод на ВСЕ семь языков интерфейса плюс пример; значение для
собственного языка — глосс-синоним, а не перевод. Новый язык добавляется
файлом плюс строкой в `StarterDecks.availableLanguages`. Приёмка —
`test/seed_translations_test.dart`: объём, раскладка, полнота переводов и
повторы (одно слово дважды допустимо только с разными значениями, как
