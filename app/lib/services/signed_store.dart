...
///
/// **Чего это НЕ даёт.** Исходники Fern открыты, и собрать сборку, где Pro
/// включён всегда, может кто угодно. Защиты от этого не существует в принципе,
/// а гнаться за ней значило бы портить жизнь честным покупателям ради тех, кто
/// платить не собирался. Задача скромнее: сделать обход не легче покупки.
class SignedStore {
  const SignedStore._();

  /// Соль подписи. В открытом коде она секретна ровно до первого желающего
  /// её прочитать — и всё же превращает правку одной строки в настройках в
  /// сборку своего APK.
