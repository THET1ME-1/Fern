...
class Fsrs {
  ...
  /// Через сколько дней вернётся слово, встреченное сегодня и оценённое [g].
  ///
  /// Это начальная прочность `w[g-1]`, переведённая в дни целевым удержанием, —
  /// ровно та величина, которую подбирает персональная оптимизация
  /// (`services/fsrs_optimizer.dart`). Считает планировщик, чтобы экран
  /// объяснения не заводил у себя вторую копию формулы.
  int firstIntervalDays(Rating g) => _intervalDays(w[g.grade - 1]);
