...
/// Тема приложения в духе Material 3 Expressive (ДНК ScoreMaster).
///
/// Тёмно-зелёная («папоротниковая») палитра, выразительная типографика
/// (Unbounded для заголовков, Onest для текста), скруглённые «таблеточные»
/// кнопки и крупные формы.
class AppTheme {
  AppTheme._();
  ...
  /// Шрифт изучаемого слова — того, что человек читает на карточке.
  ///
  /// Отдельная роль, потому что требование к нему другое: буквы обязаны
  /// различаться. В Unbounded заглавная `I`, строчная `l` и единица — три
  /// одинаковые палки, и карточка со словом `I` не читалась вовсе. У Plex
  /// заглавная I носит засечки, строчная l — хвост. Вес в ассетах только
  /// 600/700: у слова нет w800, максимум Plex — Bold.
  static const String wordFont = 'IBMPlexSans';

  /// Зелёный seed-цвет по умолчанию, из которого строится вся схема,
  /// пока пользователь не выбрал свой в настройках.
  static const Color defaultSeed = Color(0xFF2E7D5B);
